...
testPin(pin){return new Promise(async(resolve,reject)=>{var xhr=new XMLHttpRequest();xhr.open("POST",`http://${this.ipaddress}/pinTester`,!0);xhr.onload=function(){resolve(xhr.responseText)}
xhr.send(pin)})}
sendConfig(config){var xhr=new XMLHttpRequest();xhr.open("POST","/sendConfig",!0);xhr.send(JSON.stringify(config))}
