...
this.WebSocket.onclose=(e)=>{console.log("Disconnected");this.emit(this,'connected:change',{connected:!1})}
this.WebSocket.onerror=(e)=>{this.emit(this,'connected:error',{error:e})}}
...
getCurrentConfig(){return new Promise(async(resolve,reject)=>{var xhr=new XMLHttpRequest();xhr.open("GET",`http://${this.ipaddress}/returnConfigData`,!0);xhr.onload=function(){resolve(JSON.parse(xhr.responseText))};xhr.send()})}
getAllWifi(){return new Promise(async(resolve,reject)=>{var xhr=new XMLHttpRequest();xhr.open("GET",`http://${this.ipaddress}/getWifi`,!0);xhr.onload=function(){resolve(JSON.parse(xhr.responseText))};xhr.send()})}
testPin(pin){return new Promise(async(resolve,reject)=>{var xhr=new XMLHttpRequest();xhr.open("POST",`http://${this.ipaddress}/pinTester`,!0);xhr.setRequestHeader('Content-type','text/plain');xhr.onload=function(){resolve(xhr.responseText)}
xhr.send(pin)})}
sendConfig(config){var xhr=new XMLHttpRequest();xhr.open("POST",`http://${this.ipaddress}/sendConfig`,!0);xhr.setRequestHeader('Content-type','text/plain');xhr.send(JSON.stringify(config))}
